    ...
    private void updateSnake() {
        ...
        switch (mDirection) {
            case EAST: {
                ...
                if(newHead.x == mXTileCount-1 && newHead.y == 1) {
                    newHead = new Coordinate(0, newHead.y);
                }
                if(newHead.x == mXTileCount-1 && newHead.y == mYTileCount-2) {
                    newHead = new Coordinate(0, newHead.y);
                }
                break;
            }
            case WEST: {
                newHead = new Coordinate(head.x - 1, head.y);
                if(newHead.x == 0 && newHead.y == 1) {
                    newHead = new Coordinate(mXTileCount-1, newHead.y);
                }
                if(newHead.x == 0 && newHead.y == mYTileCount-2) {
                    newHead = new Coordinate(mXTileCount-1, newHead.y);
                }
                break;
            }
        ...
        if ((newHead.x < 1 && newHead.y != 1 && newHead.y != mYTileCount-2) || (newHead.y < 1) || (newHead.x > mXTileCount - 2 && newHead.y != 1 && newHead.y != mYTileCount - 2)
